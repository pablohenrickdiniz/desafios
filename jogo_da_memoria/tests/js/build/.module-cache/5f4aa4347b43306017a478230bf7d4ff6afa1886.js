var Jogo = React.createClass({displayName: "Jogo",
    ...
    carregarBaralho: function (callback) {
        ...
        $.ajax({
            ...
            success: function (options) {
                ...
                React.render(
                    React.createElement(LoadModal, {progress: 0, open: true, message: "carregando imagens..."}),
                ...
                ResourceLoader.loadImages(imagens,function(progress){
                    React.render(
                        React.createElement(LoadModal, {progress: progress, message: "carregando imagens..."}),
                        document.getElementById('carregar-modal-container')
                    );
                },function(){
                    ResourceLoader.loadAudios(audios,function(progress){
                        React.render(
                            React.createElement(LoadModal, {progress: progress, message: "carregando efeitos sonoros..."}),
                            document.getElementById('carregar-modal-container')
                        );
                    },function(){
                        React.render(
                            React.createElement(LoadModal, {progress: 100, message: "carregamento concluído!", open: false}),
                            document.getElementById('carregar-modal-container')
                        );
                        self.setState({
                            baralho:baralho
    ...
    render: function () {
        ...
        self.state.matriz.forEach(function (cartao, index) {
            if (_.isObject(cartao)) {
                ...
                cartoes.push(React.createElement(Cartao, React.__spread({},  cartao, {key: key, index: index, altura: altura, largura: largura, onClick: self.selecionarCartao, ocupado: self.state.ocupado})));
            }
        });
        ...
        var container = "";
        if(cartoes.length > 0){
            time =     React.createElement("div", {className: "row"}, 
                React.createElement("div", {className: "col-md-12"}, 
                    React.createElement("span", {className: "jogo-info"}, "Tempo: ", this.state.tempo.format('HH:mm:ss'))
                )
            );
            container =    React.createElement("div", {className: "memoria-container row", style: style}, cartoes);
        }
                                ...
                                self.setState({
                                    ...
                                    if (self.isCompleto()) {
                                        ...
                                        var props = {
                                            id: 'alert-modal',
                                            open: true,
                                            message: React.createElement("div", {className: "form-group"}, 
                                                React.createElement("span", {className: "jogo-info"}, 
                                        ...
                                        React.render(
                                            React.createElement(AlertModal, React.__spread({},  props)),
